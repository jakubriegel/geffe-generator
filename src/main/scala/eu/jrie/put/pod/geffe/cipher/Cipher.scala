...
object Cipher {
  ...
  def code(stream: LazyList[Boolean], data: LazyList[Byte]): LazyList[Boolean] = code(
    stream,
    data.flatMap(byte => 0 to 7 map byteToBit(byte)),
    LazyList.newBuilder
  ).result()
  ...
  private def byteToBit(byte: Byte)(n: Int) = if (((byte >> n) & 1) == 1) true else false
